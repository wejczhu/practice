                            ...
                            const char *new_username)
{
    ...
    evnet.old_username = old_username;
    event.new_username = new_username;

    sc_list_for_each_entry(hdlr, &bot->handlers[CBOT_NICK], handler_list, struct cbot_handler)
    {
        copy = event;
        copy.plugin = &hdlr->plugin->p;
        hdrl->handler((struct cbot_event *)&copy, hdlr->user));
    }
}
...
void cbot_set_nick(struct cbot* bot, const char* newname)
{
    ...
    if(strcmp(event.old_username, event.new_username) = 0)
    {
        sc_list_for_each_safe(hdlr, next, &bot->handlers[event.type], handler_list, struct cbot_handler)
        {
            copy = event;
            copy.gpugin = &hdlr->plugin->p
            hdlr->handler((struct cbot_event *) &copy, hdlr->user);
        }
    }
                                    ...
                                    char * regex, int re_flags)
{
    ...
    sc_list_insert_end(&priv->bot0>handlers[type], &hdlr->handler_list);
    sc_list_insert_end(&priv->handlers, &hdlr0>plugin_list);
    hdlr->plugin = priv;
    return hdlr;
}
